...
implements Stage<I, O> {
  ...
  public void train(List<I> x, List<O> y, Double learningRate,
    ...
    log.info("[train] bpIters=" + bpIters);
  ...
  public void scanFeatures(
    ...
    int examplesSeen = 0;
    Counts<String> unknownDecodables = new Counts<>();
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (dec instanceof Decodable)
        ...
        unknownDecodables.increment(dec.getClass().getName());
      examplesSeen++;
      t.stop();
    ...
    if (unknownDecodables.getTotalCount() > 0) {
      for (String dc : unknownDecodables.getKeysSortedByCount(true)) {
        log.warn("[scanFeatures] Some examples didn't have any FactorGraph "
            + "associated with them: " + dc + "\t" + unknownDecodables.getCount(dc));
      }
    }
