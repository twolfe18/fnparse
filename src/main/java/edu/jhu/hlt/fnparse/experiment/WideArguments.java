...
public class WideArguments {
  ...
  public static void measureAvgArgWidth() {
    Counts<String> sumWidth = new Counts<>();
    Counts<String> countRole = new Counts<>();
    File output = new File("/tmp/fr-widths.txt");
    ...
    for (FNParse p : parses) {
      for (FrameInstance fi : p.getFrameInstances()) {
        ...
        for (int k = 0; k < K; k++) {
          ...
          String role = f.getName() + "." + f.getRole(k);
          sumWidth.update(role, arg.width());
          countRole.increment(role);
        }
      }
    }

    try (FileWriter fw = new FileWriter(output)) {
      for (String fr : countRole.getKeysSortedByCount(true)) {
        int nFr = countRole.getCount(fr);
        int sw = sumWidth.getCount(fr);
        System.out.println(fr + "\t" + nFr + "\t" + sw);
        double avgWidth = ((double) sw) / nFr;
        fw.write(fr + "\t" + avgWidth + "\n");
      }
    } catch (Exception e) {
  ...
  public static void measureBucketedArgWidth() {
