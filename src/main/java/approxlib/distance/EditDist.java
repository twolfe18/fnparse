...
*---+ 'f7' 
	...
	public String printEditScript() {
		...
		editScript = editString.toString();
		if (editScript.length() > 0)
		  editScript = editScript.substring(0, editString.length()-1);
		
		editString = new StringBuilder();
		for (int k=compactEditList.size()-1; k>=0; k--)
			editString.append(compactEditList.get(k)+";");
		compactEditScript = editString.toString();
		if (compactEditScript.length() > 0)
		  compactEditScript = compactEditScript.substring(0, editString.length()-1);
		return editScript;
	}
...
//				break;
	...
	public String printHumaneEditScript() {
		...
		humaneEditScript = sb.toString();
		if (humaneEditScript.length() > 1)
		  humaneEditScript = humaneEditScript.substring(0, sb.length()-1);
