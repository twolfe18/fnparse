...
INPUT_FACTS=`readlink -f $1`
OUTPUT_FACTS=`readlink -f $2`
...
time zcat $INPUT_FACTS | $FNPARSE/scripts/uberts/multidoc2conll.py /dev/stdin /dev/stdout \
  | awk -F"\t" 'BEGIN{OFS="\t"} {if(NF==10){$5="_"}; print}' >$INTERMEDIATE_CONLL_X
else
time $FNPARSE/scripts/uberts/multidoc2conll.py $INPUT_FACTS /dev/stdout \
  | awk -F"\t" 'BEGIN{OFS="\t"} {if(NF==10){$5="_"}; print}' >$INTERMEDIATE_CONLL_X
fi
...
time (cd $FNPARSE/../tensorflow/models/syntaxnet && \
