...
def build_feature_set(raw_feature_file, output_ff=None, budget=10, sim_thresh=5.0, show=False):
  ...
  fs = []
  for feat in Feature.from_file(raw_feature_file):
    sim_max = (None, 0.0)
    for f in fs:
...
if __name__ == '__main__':
  if len(sys.argv) != 4:
    ...
    print '1) an input feature IG file, e.g. scripts/having-a-laugh/ig-feat.txt'
    print '2) a budget size (number of features to output -- more than 100 is slow)'
    print '3) an output file to dump feature set to'
    sys.exit(1)
  #ff = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'ig-feat.txt')
  ff = sys.argv[1]
  b = int(sys.argv[2])
  of = sys.argv[3]
  build_feature_set(ff, output_ff=of, budget=b)
