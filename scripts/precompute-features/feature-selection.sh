...
CMD="zcat $WD/templates/ig-files/pos.unigram.byFrame-shard-*-of-${NUM_SHARDS}.txt.gz >gzip -c $FEAT_POS_BYFRAME_ALL"
echo $CMD
J_FEAT_POS_BYFRAME_ALL=`qsub -hold_jid $J_POS \
  -l "h_rt=72:00:00,num_proc=1,mem_free=2G" "$CMD" | get-sge-job-id`
...
# max+score               0 1 0 1 0
# max+rank                1 0 0 0 1
mkdir $WD/templates/best-features
for C_EXP in 0 1; do
for C_MAX in 0 1; do
...
J_POS_REDUCED=`qsub -hold_jid $J_FEAT_POS_BYFRAME_ALL \
  -l 'h_rt=72:00:00,num_proc=2,mem_free=18G' "$CMD" | get-sge-job-id`
...
echo $CMD
J_POS_AND_NEG_DEDUP=`qsub -hold_jid $J_POS_AND_NEG "$CMD" | get-sge-job-id`
...
TEMPLATE_IG_FILE=$WD/templates/combined/posAndNeg-${C_EXP}${C_MAX}${C_BOTH}${C_ABS}${C_RBC}.dedup.txt # INPUT
...
for I in `seq $NUM_SHARDS | awk '{print $1 - 1}'`; do
  ...
  # This computes pos and neg already, no need to call twice
